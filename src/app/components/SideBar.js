...
export default function Sidebar() {
	...
	return (
		<div className="h-screen flex fixed lg:xl:sticky z-50 top-24">
			{isSidebarVisible && (
				<div
					className="h-screen w-60 bg-[#093967] text-white p-4"
					style={{ position: "relative" }}
				>
					<div className="flex items-center justify-between mb-4">
						<div className="flex items-center">
							<HiOutlineUser className="mr-2" />
							<button className="hover:bg-white hover:text-[#093967] transition-colors duration-300">
								Hello User
							</button>
						</div>
						<HiOutlineBell />
			...
			<button
				...
				className={`px-2 py-10 text-white rounded-md h-23 w-8 mb-40 bg-[#093967] ${
					isSidebarVisible ? "relative left-0 h-15" : "absolute left-0 h-15"
				} self-center`}
			>
				<HiOutlineArrowNarrowRight
					className={`${
						isSidebarVisible ? "transform rotate-180 text-white" : ""
					}`}
				/>
			</button>
		</div>
